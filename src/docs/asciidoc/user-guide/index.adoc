...
* Fix an annoyance in `InstanceOfAssertFactory` where `URL` and `URI` constants have been renamed to `URL_TYPE` and `URI_TYPE` respectively to avoid a clash with `java.net.URL` and `java.net.URI`. See https://github.com/joel-costigliola/assertj-core/issues/1567 for details.
* Update ByteBuddy to version 1.10.0.
* Fixe some javadoc typos
* Enforce banned dependencies with `maven-enforcer-plugin`
