...
- Add `isAbstract` to `Class` assertions. (epeee)
...
- Add `noneSatisfy(BiConsumer)` to `Map` assertions. (epeee)
...
- `anySatisfy` nad `noneSatisfy` now reports all failing elements. (epeee)
