...
Some assertions avaiable before this change can be migrated using the following mapping:

[%header,cols=2*]
|===
|Assertion before 3.17.0
|Assertion after 3.17.0
...
The remaining assertions do not have a replacement and we are happy to receive feature requests if there is a use case for any of them.
