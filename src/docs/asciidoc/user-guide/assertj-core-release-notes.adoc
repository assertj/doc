...
Thanks to all the contributors of this release: Erhard Pointl, Stefano Cordio, Pascal Schumacher, Arsen Ibragimov, Kim S. Ly, Victor Wang, Omar Morales, Reto Weiss, Michael Florian Grafl, Sergei Tachenov, mayralgr, Eveneko, Julieta, Michael Keppler, Alex Dukhno, Himadri Mandal and Jin Kwon.
...
* hasFieldOrProperty
* hasFieldOrPropertyWihtValue
* extracting field by name

Probably not an impactful breaking change: abstract classes constructor are now protected instead of public.

[[assertj-core-3.19.0-new-features]]
[.release-note-category]#icon:plus-circle[] New features#

* Add link:#assertj-core-3.19.0-isEqualToNormalizingUnicode[isEqualToNormalizingUnicode] CharSequence assertion. (Julieta)
...
* Add overloaded link:#assertj-core-3.19.0-boxed-arrays-contains-assertions[`contains` assertions with boxed arrays] to primitive array assertions. (Stefano Cordio,  mayralgr, Omar Morales)
...
* Better identation consistency in error messages.
...
* Return generic return type for `AbstractSoftAssertions.fail` for convenience and to be consistent with `Assertions.fail`
...
* Internal: Reduce test flakyness. (Erhard Pointl)
...
* Make `Collection` extending `AtomicInteger` to be representated as a collection instead of an `AtomicInteger`.
...
----
// assertions succeed
assertThat(42L).doesNotHaveToString(2501L);
assertThat("The Force").doesNotHaveToString(null);

// assertions fail
assertThat(42L).doesNotHaveToString(42L);
assertThat("The Force").doesNotHaveToString("The Force");
assertThat(new Jedi("Yoda", "Blue")).doesNotHaveToString(new Jedi("Yoda", "Blue")); 
----
