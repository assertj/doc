...
* Exclude `net.bytebuddy.experimental` property from shading scope: ensures that setting this property enables ByteBuddy experimental features also inside AssertJ. Before this change, ByteBuddy experimental features in AssertJ could only be enabled setting the `org.assertj.core.internal.bytebuddy.experimental` property.
