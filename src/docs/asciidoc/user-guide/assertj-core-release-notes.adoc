...
- Add link:#assertj-core-3.15.0-succeedsWithin[`succeedsWithin`] to `CompletableFuture` assertions. (Nikolaos Georgiou)
...
- Show explicitely if a class is package private in `ClassModifierShouldBe` error message.
...
[.release-note-category]#icon:ban[] Dep recated#
